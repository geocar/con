...
int main(int argc,char *argv[]){I Y,X,p,f,c,d=0,n[5],j,h;C b[4096];K x,y,H;struct pollfd P[2]={0};struct termios G0,G1;
  ...
  h=r1(H)->n;Y=X=0;x=nk(0);p=2;while(-1!=poll(P,2,-1))
    if(P[0].revents&POLLIN)switch(c=i0()){
      ...
      case 16:up:if(!h)break;if(h==H->n)jk(&H,r1(x));else kK(H)[h]=x;--h;x=kK(H)[h];goto redraw; case 14:down:if((h+1)>=H->n)break;kK(H)[h]=x;++h;x=kK(H)[h];goto redraw;
      case 27:switch((c=i0())){
      ...
      case '\r':case '\n':c=(xn==3&&xC[2]=='\\');if(!c&&xn>2)k(-f,l[L=='k'],r1(x),0,0);if(h==H->n)jk(&H,r1(x));else kK(H)[h]=x; h=H->n; if(c)L=*l[L!='k']; x=nk(1);p=2;break;
