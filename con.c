...
int main(int argc,char *argv[]){I f,c,d=0;C b[4096];K x,y;struct pollfd p[2]={0};struct termios G0,G1;
  ...
  x=nk();while(-1!=poll(p,2,-1))
    if(p[0].revents&POLLIN)switch(c=i0()){
      case 3:case 4:case -1:shutdown(f,SHUT_WR);break;case 12:rk(x),d=0;break;case 8:case 127:o1s("\b \b");xn--;break;
      case '\r':case '\n':if(xn>2)k(-f,"{[x];L::x;a:.q.show;.q.show::{x .Q.s y;}[neg[.z.w]];@[{.q.show value x;};x;{0N!x}];.q.show::a;}",x,0,0);else r0(x); o1s("\r\n");x=nk();break;
