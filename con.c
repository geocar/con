...
ZS sd(S p,I n){if(n>=10)p=sd(p,n/10);*p='0'+(n%10);R p+1;}
ZI sw(V){struct winsize ws={0};ioctl(0,TIOCGWINSZ,&ws);if(!ws.ws_col)ioctl(1,TIOCGWINSZ,&ws);R ws.ws_col;}
...
ZV rp(K x){o1s("\x1b[0J");if('k'==L)o1s("  ");else o1(xC,2);}
ZK nk(I cr){K x=ktn(KC,2);kC(x)[1]=')';kC(x)[0]=L;if(cr)o1s("\r\n");o1s("\x1b[6n");R x;}

//position fd(kdb) char displaymode
int main(int argc,char *argv[]){I Y,X,p,f,c,d=0;C b[4096];K x,y;struct pollfd P[2]={0};struct termios G0,G1;
  ...
  Y=X=0;x=nk(0);p=2;while(-1!=poll(P,2,-1))
    if(P[0].revents&POLLIN)switch(c=i0()){
      case 3:case -1:shutdown(f,SHUT_WR);break;
      case 12:redraw:mc(Y,X);rp(x);o1(xC+2,xn-2);d=0;gotoxy:if(p!=xn){I w=sw();if(w>0)mc(Y+(p/w),X+(p%w));else{mc(Y,X);rp(x);o1(xC+2,p-2);}}break;
      ...
      case 27:if('['==(c=i0())){I n[5]={0},j=0;do{while((c=i0())>='0'&&c<='9'&&j<5)n[j]=(c-'0')+(n[j]*10);++j;}while(c==';');switch(c){
        case 'R':Y=n[0],X=n[1];goto redraw;case 'D':goto back;case 'C':goto forward;case '~':if(n[0]==3)goto del;
      }};break;
      case '\r':case '\n':c=(xn==3&&xC[2]=='\\');if(!c&&xn>2)k(-f,l[L=='k'],x,0,0);else r0(x); if(c)L=*l[L!='k']; x=nk(1);p=2;break;
