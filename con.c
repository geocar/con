...
ZI Y,X,p,f,c,n[5],j,hp,L='q';ZC b[4096];ZK x,y,h;static struct pollfd P[2]={0};static struct termios G0,G1;ZS l[]={
...
ZV rp(V){o1s("\x1b[0J");if('k'==L)o1s("  ");else o1(xC,2);} ZK nk(V){K x=ktn(KC,2);kC(x)[1]=')';kC(x)[0]=L;R x;}
...
int main(int argc,char *argv[]){
  ...
  P->fd=0;P[1].fd=f;af(0,1);af(1,0);r1(h);hp=hn;Y=X=0;x=nk();p=2;o1s("\r\x1b[6n");while(-1!=poll(P,2,-1))
    if(P[0].revents&POLLIN)switch(c=i0()){
      ...
      case 12:redraw:mc(Y,X);rp();o1(xC+2,xn-2);af(1,0);gotoxy:if(p!=xn){I w=iw();if(w>0)mc(Y+(p/w),X+(p%w));else{mc(Y,X);rp();o1(xC+2,p-2);}}break;
      ...
      case '\r':case '\n':c=(xn==3&&xC[2]=='\\');if(!c&&xn>2)k(-f,l[L=='k'],r1(x),0,0);if(hp==hn)jk(&h,r1(x));else kK(h)[hp]=x; hp=hn; if(c)L=*l[L!='k']; x=nk();p=2;af(1,0);break;
